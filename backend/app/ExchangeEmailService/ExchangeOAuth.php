...
class ExchangeOAuth
{
    ...
    public function __construct($tenantId = null, $clientId = null, $clientSecret = null, $redirectUri = null, $scope = null, $authMethod = null, $fromEmail = null, $fromName = null)
    {
        $this->tenantId = $this->nonEmpty($tenantId) ?? $this->configValue('tenant_id');
        $this->clientId = $this->nonEmpty($clientId) ?? $this->configValue('client_id');
        $this->clientSecret = $this->nonEmpty($clientSecret) ?? $this->configValue('client_secret');
        $this->redirectUri = $this->nonEmpty($redirectUri) ?? $this->configValue('redirect_uri');
        $this->scope = $this->nonEmpty($scope) ?? $this->configValue('scope') ?? 'https://graph.microsoft.com/.default';
    ...
    private function nonEmpty(mixed $value): mixed
    {
        if ($value === null || $value === '') {
            return null;
        }
    ...
    private function configValue(string $key): mixed
    {
        ...
        return config('exchange-email.'.$key);
    }
}
